...
export function EcgAuthGate({initialAuthenticated,configured,canonicalMeasurementsEnabled=false,canonicalRateRhythmEnabled=false}:{initialAuthenticated:boolean;configured:boolean;canonicalMeasurementsEnabled?:boolean;canonicalRateRhythmEnabled?:boolean}){
  ...
  if(authenticated)return <><div className="auth-toolbar"><span>限定公開・認証済み</span><button className="btn secondary" type="button" onClick={logout} disabled={busy}>ログアウト</button></div><EcgWorkspace onAuthRequired={()=>{setAuthenticated(false);void checkConfiguration()}} canonicalMeasurementsEnabled={canonicalMeasurementsEnabled} canonicalRateRhythmEnabled={canonicalRateRhythmEnabled}/></>;
