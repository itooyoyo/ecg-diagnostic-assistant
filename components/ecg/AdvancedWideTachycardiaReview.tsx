...
export function AdvancedWideTachycardiaReview(p:Props){
  ...
  return <details className="tachy-inputs wct-review" id="wide-qrs-tachy-review" open>
    ...
    {brugadaAvailable&&<section className="wct-layer"><h4>2. Brugada WCT鑑別項目（監査用・Rule未接続）</h4><p className="muted">入力内容は保持されますが、診断候補・緊急度・59 Rulesの結果は変更しません。</p><div className="tachy-form-grid">
      <label>V1〜V6すべてでRS complexなし<select aria-label="V1からV6すべてでRS complexなし" value={p.audit.rsAbsentV1V6} onChange={e=>audit({rsAbsentV1V6:e.target.value as WctAuditState["rsAbsentV1V6"]})}><option value="unknown">未入力</option><option value="yes">はい（全誘導でRSなし）</option><option value="no">いいえ（1誘導以上でRSあり）</option><option value="indeterminate">判定困難</option></select></label>
      ...
      <label>最大RS interval（ms）<input aria-label="最大RS interval ミリ秒" type="number" min="0" inputMode="numeric" placeholder="例：120" value={p.audit.maxRsIntervalMs??""} disabled={p.audit.rsIntervalStatus!=="measured"} onChange={e=>audit({maxRsIntervalMs:e.target.value===""?null:Number(e.target.value)})}/><small>R波開始からS波最深点まで。25 mm/sでは1 mm = 40 ms。</small></label>
    ...
    </div><details className="wct-education"><summary>見方：RS complex・RS interval・BBB様形態・concordance</summary><div className="wct-education-grid"><Card title="RS complex" text="同一QRS内にR波と、それに続くS波があるかをV1〜V6で1誘導ずつ確認します。QSや単相性RはRS complexではありません。1誘導でもRSがある場合は『全誘導でRSなし』を選びません。見分けられない誘導があれば『判定困難』を選択します。"><Diagram kind="rs"/></Card><Card title="RS interval" text="RS complexがある胸部誘導で、R波開始からS波最深点までを測り、最も長い値を入力します。25 mm/sでは1 mm = 40 msです。開始点または最深点が不明瞭なら数値を入れず、『測定不能』または『判定困難』を選択します。"><Diagram kind="interval"/></Card><Card title="BBB様形態" text="頻拍時QRSがRBBB-like、LBBB-like、どちらとも言えない、判定困難のどれかを記録します。典型形態との比較に確信がない場合は、無理にRBBB-like／LBBB-likeを選びません。"><Diagram kind="rs"/></Card><Card title="Precordial concordance" text="V1〜V6の主QRS方向がすべて陽性ならpositive、すべて陰性ならnegative、陽性と陰性が混在すればmixedです。主方向または誘導を評価できなければ『判定困難』を選択します。"><Diagram kind="concordance"/></Card></div></details>{(p.audit.rsAbsentV1V6!=="unknown"||p.audit.rsIntervalStatus!=="unentered"||p.audit.bbbLikeMorphology!=="unknown"||p.audit.concordance!=="unknown")&&<p className="wct-audit-status">監査入力済み（診断Rule未接続・診断結果と緊急度には影響しません）</p>}</section>}
  </details>
}
