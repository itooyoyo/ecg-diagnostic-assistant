...
import { classifyTachyarrhythmia, type PWaveState, type Regularity, type TachyInput } from "@/logic/tachyarrhythmia/classify.js";
...
export function TachyarrhythmiaModule({heartRate,qrsMs,regularity,onRedFlagChange}:TachyarrhythmiaModuleProps) {
  ...
  const [pQrs,setPQrs]=useState("unknown");
  ...
  const input:TachyInput={heartRate,qrsMs,regularity,pWave,pulsePresent,systolicBp,...flags,wpwHistory,qrsMorphologyVariable,priorMi,structuralHeartDisease,sinusFeatures,qtcMs,potassium,calcium,magnesium};
  ...
  return <section className="card tachy-shell" id="tachyarrhythmia">
    ...
    {(result.hemodynamics.status==="unstable"||result.hemodynamics.status==="cardiac-arrest")&&<div className="tachy-redflag" role="alert"><div className="eyebrow">Red Flag</div><strong>{result.hemodynamics.label}</strong><p>{result.hemodynamics.message}</p>{result.hemodynamics.status==="unstable"&&<small>同期の可否と鎮静可能性を確認します。具体的エネルギー値は提示しません。</small>}</div>}

    <div className="tachy-flow" aria-label="頻脈分類フロー"><span>01 循環動態</span><i>→</i><span>02 QRS幅</span><i>→</i><span>03 規則性</span><i>→</i><span>04 候補</span></div>
    ...
    <details className="tachy-inputs"><summary>P波・頻拍機序を確認</summary>
      <div className="tachy-form-grid">
        ...
        <label>P–QRS関係<select value={pQrs} onChange={e=>setPQrs(e.target.value)}><option value="unknown">不明</option><option value="one-to-one">PとQRSが1:1</option><option value="more-p">PがQRSより多い</option><option value="more-qrs">QRSがPより多い</option><option value="av-dissociation">房室解離</option></select></label>
      </div>
