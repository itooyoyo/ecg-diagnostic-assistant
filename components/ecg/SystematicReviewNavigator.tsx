...
type Status="unreviewed"|"normal"|"abnormal"|"indeterminate"|"not_applicable";
...
const steps:Step[]=[
 ...
 {id:"rate",title:"心拍数・規則性",hint:"心房拍数と心室拍数を分けて確認",items:["心拍数","心房拍数","心室拍数","regular","regularly irregular","irregularly irregular"],signals:[{key:"sinusRhythm",label:"洞調律候補"},{key:"regular",label:"regular"},{key:"irregularlyIrregular",label:"irregularly irregular"},{key:"vf",label:"VF候補"},{key:"sustainedVt",label:"持続性VT候補"}]},
 ...
 {id:"summary",title:"総合確認",hint:"未確認・矛盾・Red Flagを確認して結果へ進む",items:["未確認項目","入力矛盾","追加確認","緊急候補"]},
];
const labels:Record<string,string>={unreviewed:"未確認",normal:"正常",abnormal:"異常",indeterminate:"判定困難",not_applicable:"該当なし"};

export function SystematicReviewNavigator(){
 ...
 const updateStatus=(id:string,status:Status)=>setState(s=>({...s,steps:{...s.steps,[id]:{...s.steps[id],status}}}));
 ...
 const toggleComplete=(id:string)=>setState(s=>({...s,steps:{...s.steps,[id]:{...s.steps[id],completed:!s.steps[id].completed}}}));
 ...
 <label className="check urgent-review"><input type="checkbox" checked={state.urgentMode} onChange={e=>setState(s=>({...s,urgentMode:e.target.checked}))}/>緊急所見を優先して結果を表示</label>
 <div aria-hidden="true">{steps.map(step=><span id={`review-${step.id}`} key={step.id}/>)}</div>
 ...
 <div className="review-step-list">{steps.map((step,index)=>{const current=state.steps[step.id];return <details className={`review-step review-step--${current.status}`} key={step.id} open={index===0&&!current.completed}><summary><span>STEP {index+1}</span><strong>{step.title}</strong><small>{labels[current.status]}</small></summary><div className="review-step-body"><p className="muted">{step.hint}</p><label>確認状態<select value={current.status} onChange={e=>updateStatus(step.id,e.target.value as Status)}>{reviewStatuses.map(x=><option value={x} key={x}>{labels[x]}</option>)}</select></label>{current.status==="abnormal"&&<div className="review-details"><h4>詳細入力</h4><ul className="compact-item-list">{step.items.map(x=><li key={x}>{x}</li>)}</ul>{step.signals?.map(x=><label className="check" key={x.key}><input type="checkbox" checked={Boolean(current.values[x.key])} onChange={e=>toggleValue(step.id,x.key,e.target.checked)}/>{x.label}</label>)}</div>}<label className="check step-complete"><input type="checkbox" checked={current.completed} onChange={()=>toggleComplete(step.id)}/>医師がこのSTEPを確認完了</label></div></details>})}</div>
