  ...
  return <div className="shell">
    ...
    <main className="main">
      ...
      <section className="card systematic-shell" id="section-6">
        ...
        <InterpretationNavigator items={builtSystematicItems} stInput={stInput} stResult={stResult} onStChange={(next)=>{setStInput(next);setSystematicItems(current=>current.map(item=>item.id==="st-change"?{...item,status:"edited"}:item))}} tWaveInput={integratedTWaveInput} tWaveResult={tWaveResult} onTWaveChange={(next)=>{setTWaveInput(next);setSystematicItems(current=>current.map(item=>item.id==="t-wave"?{...item,status:"edited"}:item))}} qtInput={qtInput} qtResult={qtResult} onQtChange={(next)=>{setQtInput(next);setSystematicItems(current=>current.map(item=>item.id==="qt-qtc"?{...item,status:"edited"}:item))}} onChange={(next)=>setSystematicItems((current)=>current.map((item)=>item.id===next.id?next:item))}/>
      </section>
