...
function ClinicalResults({result,reviewDisplay,pearls}:{result:IntegratedResult;reviewDisplay:{enteredFindings:string[];unassessedItems:string[]};pearls:string[]}) {
  ...
  return <section className="card workflow-card clinical-results" id="clinical-results"><div className="cardhead"><div><div className="eyebrow">Step 3 · Result</div><h3>診断・対応</h3><p className="muted">医師確認所見から再計算した結果です。</p></div><span className={`urgency-chip urgency-chip--${result.urgency}`}>{result.urgency}</span></div>
    ...
    <ResultBlock number="1" title="診断候補（優先順位付き）">{result.diagnosticCandidates.length?<div className="candidate-reasons">{result.diagnosticCandidates.map((x,i)=><article key={x.id}><header><span>{i+1}</span><div><strong>{x.label}</strong><small>Rule confidence：{ruleConfidenceJa(x.confidence)} · {x.urgency}</small></div></header><div><b>判定理由</b><SimpleList items={x.supportingFindings.map(v=>v.label)}/></div><details><summary>詳細Rule情報</summary><p><strong>使用Rule：</strong>{x.ruleIds.join("、")||"なし"}</p><p><strong>除外理由</strong></p><SimpleList items={x.contradictingFindings.map(v=>v.label)}/><p><strong>不足情報</strong></p><SimpleList items={x.missingInformation.map(v=>v.label)}/></details></article>)}</div>:<p className="muted">現時点で優先度の高い診断候補はありません。</p>}</ResultBlock>
