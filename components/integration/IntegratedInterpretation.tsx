...
export function IntegratedInterpretation({result,override,onOverrideChange}:{result:Result;override:IntegratedOverride;onOverrideChange:(x:IntegratedOverride)=>void}){
  return <section className="card integrated-shell" id="integrated-summary">
    ...
    <details className="integrated-section"><summary>Rule ID統合情報</summary><dl><div><dt>発火Rule ID</dt><dd>{result.ruleRelations.matchedRuleIds.join("、")||"なし"}</dd></div><div><dt>不足Rule ID</dt><dd>{result.ruleRelations.insufficientRuleIds.join("、")||"なし"}</dd></div><div><dt>競合Rule ID</dt><dd>{result.ruleRelations.competingRuleIds.join("、")||"なし"}</dd></div><div><dt>抑制Rule ID</dt><dd>{result.ruleRelations.suppressedRuleIds.join("、")||"なし"}</dd></div><div><dt>関連Rule ID</dt><dd>{result.ruleRelations.relatedRuleIds.join("、")||"なし"}</dd></div></dl></details>
