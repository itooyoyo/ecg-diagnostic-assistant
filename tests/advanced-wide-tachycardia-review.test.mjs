...
test("regular monomorphic Wide exposes the Advanced WCT route",()=>{assert.match(workspace,/result\.qrsClass\s*===\s*"wide"/);assert.match(ui,/Wide QRS頻拍：VT \/ SVT鑑別/);assert.match(ui,/polymorphicWide===false/)});
...
test("RS absence explains that one RS lead makes the all-absent answer no",()=>{assert.match(ui,/1誘導でもRSがある場合は『全誘導でRSなし』を選びません/);assert.match(ui,/いいえ（1誘導以上でRSあり）/)});
test("RS interval exposes the measurement landmarks and existing paper speed constant",()=>{assert.match(ui,/R波開始からS波最深点まで/);assert.match(ui,/25 mm\/sでは1 mm = 40 ms/);assert.match(ui,/rsIntervalStatus!=="measured"/)});
test("RS interval can remain unmeasurable or indeterminate without a number",()=>{assert.match(ui,/value="unmeasurable">測定不能/);assert.match(ui,/value="indeterminate">判定困難/);assert.match(ui,/e\.target\.value==="measured"\?p\.audit\.maxRsIntervalMs:null/)});
test("BBB-like morphology keeps neither and indeterminate as formal choices",()=>{assert.match(ui,/value="neither">どちらとも言えない/);assert.match(ui,/value="indeterminate">判定困難/);assert.match(ui,/無理にRBBB-like／LBBB-likeを選びません/)});
test("concordance education defines positive negative mixed and indeterminate",()=>{assert.match(ui,/すべて陽性ならpositive、すべて陰性ならnegative、陽性と陰性が混在すればmixed/);assert.match(ui,/主方向または誘導を評価できなければ『判定困難』/)});
test("audit status explicitly states no diagnostic or urgency effect",()=>assert.match(ui,/診断Rule未接続・診断結果と緊急度には影響しません/));
