...
test("PNG image is accepted",()=>assert.equal(validateEcgFile({type:"image/png",size:1024}).valid,true));
test("WebP image is accepted",()=>assert.equal(validateEcgFile({type:"image/webp",size:1024}).valid,true));
test("PDF is rejected while PDF analysis is unavailable",()=>assert.equal(validateEcgFile({type:"application/pdf",size:1024}).valid,false));
test("empty image is rejected",()=>assert.equal(validateEcgFile({type:"image/png",size:0}).valid,false));
...
const integrated=(patch={})=>{const x=createDefaultIntegratedInput();for(const [group,values] of Object.entries(patch)){if(typeof values==="object"&&values!==null&&!Array.isArray(values))Object.assign(x[group],values);else x[group]=values}return x};
const ids=r=>r.diagnosticCandidates.map(x=>x.id);
test("integration 01: normal-quality asymptomatic input has no critical finding and retains limits",()=>{const r=buildIntegratedInterpretation(integrated());assert.equal(r.criticalFindings.length,0);assert.match(r.summaryText.join(" "),/緊急性の高い所見は抽出されていません/);assert.ok(r.limitations.length)});
test("integration 02: inadequate quality is not normal and prioritizes repeat ECG",()=>{const r=buildIntegratedInterpretation(integrated({quality:{imageAdequate:false}}));assert.equal(r.dataQuality,"inadequate");assert.equal(r.urgency,"uncertain");assert.equal(r.todaysPlan[0].id,"repeat-ecg")});
test("integration 03: placement concern plus ST change retains ACS and repeat ECG",()=>{const r=buildIntegratedInterpretation(integrated({quality:{placementConcern:true},clinical:{ischemicChestPain:true},ecg:{contiguousStElevation:true}}));assert.ok(ids(r).includes("acute-coronary-occlusion"));assert.ok(r.conflictingFindings.some(x=>x.id==="ischemia-placement"))});
test("integration 04: ST elevation reciprocal change and pain flags occlusion",()=>assert.ok(buildIntegratedInterpretation(integrated({clinical:{ischemicChestPain:true},ecg:{contiguousStElevation:true,reciprocalChange:true}})).criticalFindings.some(x=>x.id==="acute-coronary-occlusion")));
test("integration 05: inferior ST elevation and hypotension proposes V4R",()=>{const r=buildIntegratedInterpretation(integrated({clinical:{hypotension:true},ecg:{inferiorStElevation:true}}));assert.ok(ids(r).includes("inferior-rv-conduction"));assert.ok(r.todaysPlan.some(x=>x.id==="right-leads"))});
test("integration 06: V1-V3 depression and pain proposes V7-V9",()=>assert.ok(buildIntegratedInterpretation(integrated({clinical:{ischemicChestPain:true},ecg:{stDepressionV1toV3:true}})).todaysPlan.some(x=>x.id==="posterior-leads")));
test("integration 07: biphasic anterior T with pain history and pain-free state retains Wellens",()=>assert.ok(ids(buildIntegratedInterpretation(integrated({clinical:{chestPainHistory:true,currentlyPainFree:true},ecg:{wellensMorphology:true}}))).includes("wellens-pattern")));
test("integration 08: negative troponin does not remove Wellens",()=>assert.ok(ids(buildIntegratedInterpretation(integrated({clinical:{chestPainHistory:true,currentlyPainFree:true,troponinNegative:true},ecg:{wellensMorphology:true}}))).includes("wellens-pattern")));
test("integration 09: diffuse depression plus aVR elevation is high-risk ischemia",()=>assert.ok(ids(buildIntegratedInterpretation(integrated({clinical:{ischemicChestPain:true},ecg:{diffuseStDepression:true,avrElevation:true}}))).includes("diffuse-subendocardial-ischemia")));
test("integration 10: QT prolongation bradycardia and PVC raises TdP risk",()=>assert.ok(ids(buildIntegratedInterpretation(integrated({ecg:{qtProlonged:true,bradycardia:true,pvc:true}}))).includes("tdp-risk")));
test("integration 11: long QT R-on-T and low-K pattern prioritizes monitoring and labs",()=>{const r=buildIntegratedInterpretation(integrated({ecg:{qtProlonged:true,rOnT:true,flattenedT:true,prominentU:true}}));assert.ok(r.todaysPlan.some(x=>x.id==="monitor"));assert.ok(r.todaysPlan.some(x=>x.id==="electrolytes"))});
test("integration 12: polymorphic wide tachycardia with long QT is TdP candidate",()=>assert.match(buildIntegratedInterpretation(integrated({ecg:{polymorphicWideTachycardia:true,qtProlonged:true}})).diagnosticCandidates.find(x=>x.id==="tdp-risk").label,/TdP/));
test("integration 13: polymorphic wide tachycardia with normal QT retains other wide tachy candidate",()=>assert.ok(ids(buildIntegratedInterpretation(integrated({ecg:{wideTachycardia:true,polymorphicWideTachycardia:true}}))).includes("wide-qrs-tachycardia")));
test("integration 14: peaked T absent P and wide QRS is severe hyperkalemia pattern",()=>assert.match(buildIntegratedInterpretation(integrated({ecg:{peakedT:true,pWaveAbsent:true,wideQrs:true}})).diagnosticCandidates.find(x=>x.id==="hyperkalemia-pattern").label,/重症高K/));
test("integration 15: hyperkalemia output excludes cause investigation",()=>assert.ok(buildIntegratedInterpretation(integrated({ecg:{peakedT:true}})).diagnosticCandidates.find(x=>x.id==="hyperkalemia-pattern").limitations.some(x=>x.includes("原因検索"))));
test("integration 16: hyperkalemia offers disabled electrolyte tool",()=>{const r=buildIntegratedInterpretation(integrated({ecg:{peakedT:true}}));assert.equal(r.externalToolActions[0].target,"electrolyte_assistant");assert.equal(r.externalToolActions[0].disabled,true)});
test("integration 17: flat T U wave and PVC suggests low K Mg pattern",()=>assert.ok(ids(buildIntegratedInterpretation(integrated({ecg:{flattenedT:true,prominentU:true,pvc:true}}))).includes("low-k-mg-pattern")));
test("integration 18: short QT with measured high calcium retains high-Ca candidate",()=>assert.ok(ids(buildIntegratedInterpretation(integrated({ecg:{qtShort:true},labs:{calcium:12}}))).includes("hypercalcemia-pattern")));
test("integration 19: long QT and ST with measured low calcium retains low-Ca candidate",()=>assert.ok(ids(buildIntegratedInterpretation(integrated({ecg:{qtProlonged:true,stProlonged:true},labs:{calcium:6}}))).includes("hypocalcemia-pattern")));
test("integration 20: electrolyte candidate generates no endocrine or renal causes",()=>{const text=JSON.stringify(buildIntegratedInterpretation(integrated({ecg:{peakedT:true}})));assert.doesNotMatch(text,/副甲状腺|アルドステロン|尿K|TTKG|腎性/)});
test("integration 21: Mobitz II wide QRS syncope is emergency",()=>assert.equal(buildIntegratedInterpretation(integrated({clinical:{syncope:true},ecg:{mobitzII:true,wideQrs:true}})).urgency,"emergency"));
test("integration 22: 2-to-1 block is not automatically Mobitz II",()=>assert.doesNotMatch(buildIntegratedInterpretation(integrated({ecg:{twoToOneBlock:true}})).diagnosticCandidates.find(x=>x.id==="advanced-av-block").label,/Mobitz II/));
test("integration 23: near-rate AV dissociation does not confirm complete block",()=>{const r=buildIntegratedInterpretation(integrated({ecg:{avDissociation:true,atrialRate:70,ventricularRate:68}}));assert.notEqual(r.diagnosticCandidates.find(x=>x.id==="advanced-av-block").label,"完全房室ブロックを疑う");assert.ok(r.diagnosticCandidates.find(x=>x.id==="advanced-av-block").contradictingFindings.length)});
test("integration 24: complete block with wide escape is emergency",()=>assert.equal(buildIntegratedInterpretation(integrated({ecg:{completeBlock:true,wideEscape:true}})).urgency,"emergency"));
test("integration 25: wide tachy AV dissociation and prior MI strongly suggests VT",()=>assert.match(buildIntegratedInterpretation(integrated({clinical:{priorMi:true},ecg:{wideTachycardia:true,avDissociationTachy:true}})).diagnosticCandidates.find(x=>x.id==="wide-qrs-tachycardia").label,/強く疑う/));
test("integration 26: wide tachy with existing LBBB retains SVT plus BBB",()=>assert.ok(buildIntegratedInterpretation(integrated({ecg:{wideTachycardia:true,existingBbb:true,lbbb:true}})).diagnosticCandidates.find(x=>x.id==="wide-qrs-tachycardia").alternativeExplanations.some(x=>x.includes("SVT"))));
test("integration 27: irregular wide tachy with preexcitation is red flag",()=>assert.ok(buildIntegratedInterpretation(integrated({ecg:{wideTachycardia:true,irregularTachycardia:true,preexcitation:true}})).criticalFindings.some(x=>x.id==="preexcited-af")));
test("integration 28: WPW finding alone does not create blanket drug warning",()=>assert.equal(buildIntegratedInterpretation(integrated({ecg:{preexcitation:true}})).diagnosticCandidates.some(x=>x.id==="preexcited-af"),false));
test("integration 29: RBBB right strain and hypoxia suggests acute right-heart strain",()=>assert.ok(ids(buildIntegratedInterpretation(integrated({clinical:{severeHypoxia:true},ecg:{rbbb:true,rightAxis:true,rvStrain:true}}))).includes("acute-right-heart-strain")));
test("integration 30: S1Q3T3 alone does not diagnose PE",()=>assert.equal(ids(buildIntegratedInterpretation(integrated({ecg:{s1q3t3:true}}))).includes("acute-right-heart-strain"),false));
test("integration 31: giant negative T with neurologic symptoms suggests CNS candidate",()=>assert.match(buildIntegratedInterpretation(integrated({clinical:{neurologicSymptoms:true},ecg:{giantNegativeT:true}})).diagnosticCandidates.find(x=>x.id==="giant-negative-t-differential").label,/中枢神経/));
test("integration 32: giant negative T with apical hypertrophy suggests apical HCM",()=>assert.match(buildIntegratedInterpretation(integrated({clinical:{apicalHypertrophy:true},ecg:{giantNegativeT:true}})).diagnosticCandidates.find(x=>x.id==="giant-negative-t-differential").label,/心尖部/));
test("integration 33: unstable hemodynamics places ABC first",()=>assert.equal(buildIntegratedInterpretation(integrated({clinical:{shock:true},ecg:{wellensMorphology:true}})).todaysPlan[0].id,"abc"));
test("integration 34: duplicate plans merge provenance",()=>{const r=buildIntegratedInterpretation(integrated({ecg:{qtProlonged:true,bradycardia:true,pvc:true,flattenedT:true,prominentU:true}}));assert.equal(r.todaysPlan.filter(x=>x.id==="monitor").length,1);assert.ok(r.todaysPlan.find(x=>x.id==="monitor").sourceModules.length>=2)});
test("integration 35: physician rejects ST finding and ACS recalculates",()=>{const x=integrated({clinical:{ischemicChestPain:true},ecg:{contiguousStElevation:true}});assert.ok(ids(buildIntegratedInterpretation(x)).includes("acute-coronary-occlusion"));x.rejectedFindingIds.push("st-elevation");assert.equal(ids(buildIntegratedInterpretation(x)).includes("acute-coronary-occlusion"),false)});
test("integration 36: physician QT correction recalculates TdP risk",()=>{const x=integrated({ecg:{qtProlonged:true,bradycardia:true,pvc:true}});assert.ok(ids(buildIntegratedInterpretation(x)).includes("tdp-risk"));x.ecg.qtProlonged=false;x.ecg.bradycardia=false;assert.equal(ids(buildIntegratedInterpretation(x)).includes("tdp-risk"),false)});
test("integration 37: physician wide-to-narrow correction recalculates tachy differential",()=>{const x=integrated({ecg:{wideTachycardia:true}});assert.ok(ids(buildIntegratedInterpretation(x)).includes("wide-qrs-tachycardia"));x.ecg.wideTachycardia=false;assert.equal(ids(buildIntegratedInterpretation(x)).includes("wide-qrs-tachycardia"),false)});
test("integration 38: urgency override preserves lower findings",()=>{const x=integrated({ecg:{completeBlock:true}}),before=buildIntegratedInterpretation(x);x.override.urgency="same_day";const after=buildIntegratedInterpretation(x);assert.equal(after.urgency,"same_day");assert.deepEqual(ids(after),ids(before));assert.equal(after.revision.modified,true)});
test("integration 39: indeterminate item is not treated as normal",()=>{const x=integrated();x.indeterminateFindingIds.push("p-wave");const r=buildIntegratedInterpretation(x);assert.match(r.limitations.join(" "),/判定不能/)});
test("integration 40: responsive UI keeps critical summary before details",()=>{const ui=readFileSync(new URL("../components/integration/IntegratedInterpretation.tsx",import.meta.url),"utf8"),css=readFileSync(new URL("../app/globals.css",import.meta.url),"utf8");assert.ok(ui.indexOf("CriticalSummary")<ui.indexOf("DiagnosticCandidates"));assert.match(css,/@media\(max-width:720px\).*integrated-grid.*grid-template-columns:1fr/s)});
test("final case: NSTEMI-like diffuse subendocardial ischemia remains an urgent candidate",()=>{const r=buildIntegratedInterpretation(integrated({clinical:{ischemicChestPain:true},ecg:{diffuseStDepression:true,avrElevation:true}}));assert.ok(ids(r).includes("diffuse-subendocardial-ischemia"));assert.equal(r.urgency,"emergency")});
test("final case: VF with cardiac arrest routes to resuscitation priority",()=>assert.equal(buildIntegratedInterpretation(integrated({clinical:{cardiacArrest:true},ecg:{vf:true}})).urgency,"resuscitation"));
test("final case: limb electrode reversal remains a technical limitation, not a normal ECG",()=>{const r=buildIntegratedInterpretation(integrated({quality:{limbReversal:true}}));assert.ok(ids(r).includes("technical-limitation"));assert.ok(r.todaysPlan.some(x=>x.id==="repeat-ecg"))});
