...
export async function analyzeLocalEcgPoc(file,{signal,onStatus,layoutType="auto"}={}){
  ...
  try{
    ...
    const gates=evaluatePocStageGates({width,height,layoutType:selectedLayout,gridDetected:Boolean(grid.detected&&grid.xPeriod&&grid.yPeriod)});
    ...
    const measurements=measure(leads,grid,imageQuality.level);
...
function measure(leads,grid,quality){const reference=leads.find(x=>x.lead==="II"&&x.quality!=="inadequate")??leads.find(x=>x.quality!=="inadequate");const peaks=reference?detectPeaks(reference.points,Math.max(4,grid.yPeriod)):[];const rrPx=peaks.slice(1).map((x,i)=>x-peaks[i]).filter(x=>x>0);const rrIntervals=rrPx.map(px=>Math.round(px/Math.max(1,grid.xPeriod)*40));const medianRr=median(rrIntervals);const heartRateBpm=medianRr&&rrIntervals.length>=2?Math.round(60000/medianRr):null;const cv=coefficientOfVariation(rrIntervals);const rhythmRegularity=rrIntervals.length<3?"indeterminate":cv<.08?"regular":cv<.18?"regularly_irregular":"irregularly_irregular";const qrs=reference&&peaks.length?estimateQrs(reference.points,peaks,grid):null;const estimatedQrsDurationMs=qrs?.ms??null;const qrsWidthCandidate=estimatedQrsDurationMs==null?"indeterminate":Math.abs(estimatedQrsDurationMs-120)<12?"indeterminate":estimatedQrsDurationMs>=120?"wide":"narrow";const stDirections=leads.map(lead=>estimateSt(lead,grid,qrs?.widthPx??null));const limitations=[];if(!heartRateBpm)limitations.push("Rピーク数が不足し、心拍数とRR規則性を判定できません。");if(qrsWidthCandidate==="indeterminate")limitations.push("QRS幅候補を安全に分類できません。");if(stDirections.some(x=>x.direction==="indeterminate"))limitations.push("ST方向を判定できない誘導があります。");return {heartRateBpm,rrIntervals,rhythmRegularity,estimatedQrsDurationMs,qrsWidthCandidate,stDirections,quality:quality==="adequate"?"adequate":"limited",limitations}}
