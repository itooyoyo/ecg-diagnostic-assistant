...
export function detectSupportedLayout(gray) {
  if (!gray?.width || !gray?.height || gray.width < 160 || gray.height < 120) return { layoutType: "unknown", confidence: "indeterminate", bandCount: 0 };
  const rowScores = [];
  for (let y = 0; y < gray.height; y += 1) {
    let dark = 0;
  ...
  if (bands >= 5 && bands <= 7) return { layoutType: "six_by_two", confidence: "medium", bandCount: bands };
  if (bands >= 3 && bands <= 4) return { layoutType: "three_by_four", confidence: "low", bandCount: bands };
  return { layoutType: "unknown", confidence: "indeterminate", bandCount: bands };
}
