...
function qrsClusterPeak(cluster,index,baseline){const candidates=cluster.components.flatMap(component=>component.pixels.map(pixel=>({x:component.originX+pixel.x,y:component.originY+pixel.y}))),point=candidates.sort((a,b)=>Math.abs(b.y-baseline)-Math.abs(a.y-baseline))[0]??cluster.components[0].centroid;return {clusterId:index,x:point.x,y:point.y,amplitude:Math.abs(point.y-baseline)}}
