...
export type PeakCandidate={x:number;prominence:number;amplitude:number;precedingRrPx:number|null;accepted:boolean;rejectionReason:string|null};
...
export type LocalPocMeasurements={heartRateBpm:number|null;heartRateSource:MeasurementSource;rrIntervals:number[];rhythmRegularity:RhythmRegularity;rhythmSource:MeasurementSource;peakCandidates:PeakCandidate[];estimatedQrsDurationMs:number|null;qrsWidthCandidate:QrsWidthCandidate;qrsAudit:{measurementLead:string|null;beatCount:number;beats:QrsBeatAudit[];medianDurationMs:number|null;quality:PocQuality;limitations:string[]};stDirections:StDirectionCandidate[];quality:PocQuality;limitations:string[]};
export type LocalPocResult={imageQuality:PocQuality;layout:"three_by_four"|"three_by_four_with_long_ii"|"six_by_two";leads:DigitizedLeadPoC[];longII:DigitizedLeadPoC|null;measurements:LocalPocMeasurements;context:IntegratedInput;ruleResult:IntegratedInterpretation;extractedFields:string[];indeterminateFields:string[];limitations:string[];processingTimeMs:number};
