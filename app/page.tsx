...
export default async function Home() {
  ...
  return <EcgAuthGate initialAuthenticated={session.status==="valid"} configured={config.configured} canonicalMeasurementsEnabled={canonicalMeasurementsEnabled(process.env.ECG_CANONICAL_MEASUREMENTS_ENABLED)} canonicalRateRhythmEnabled={canonicalRateRhythmEnabled(process.env.ECG_CANONICAL_RATE_RHYTHM_ENABLED)} canonicalBasicRhythmCandidatesEnabled={canonicalBasicRhythmCandidatesEnabled(process.env.ECG_CANONICAL_BASIC_RHYTHM_CANDIDATES_ENABLED)}/>;
}
